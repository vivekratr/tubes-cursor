...
import { hexToRgb, themeFromEl } from "./data";
...
function cacheAccents(root: HTMLElement) {
  const cs = getComputedStyle(root);
  return {
    blue: hexToRgb(cs.getPropertyValue("--tc-accent-blue")),
    green: hexToRgb(cs.getPropertyValue("--tc-accent-green")),
  };
}

function loadLogo(img: HTMLImageElement, src: string) {
...
function startDna(opts: DnaOpts) {
  const { field, mark, pulse, sign, root } = opts;
  const maybeCtx = field.getContext("2d", { alpha: true });
  if (!maybeCtx) return () => {};
  ...
  function sample() {
    ...
    const pts: { tx: number; ty: number; r: number; g: number; b: number }[] =
      [];
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      for (let j = i + 1; j < particles.length; j++) {
        const d = Math.hypot(
          particles[i].tx - particles[j].tx,
          particles[i].ty - particles[j].ty,
        );
        if (d < bestD && d > 4) {
          bestD = d;
          best = j;
  ...
  function posAt(p: Particle, elapsed: number) {
  ...
  function kick() {
    if (looping || !alive) return;
    looping = true;
    raf = requestAnimationFrame(draw);
  }

  function pointerToSign(e: PointerEvent) {
  ...
  function onMove(e: PointerEvent) {
    ...
    mark.style.setProperty(
      "--hx",
      `${((e.clientX - mr.left) / mr.width) * 100}%`,
    );
    mark.style.setProperty(
      "--hy",
      `${((e.clientY - mr.top) / mr.height) * 100}%`,
    );
    kick();
  }
  ...
  function draw(now: number) {
    ...
    const settled = particles.reduce(
      (n, p) => n + (elapsed - p.delay > p.dur ? 1 : 0),
      0,
    );
    const ratio = particles.length ? settled / particles.length : 1;
    const intro = elapsed < INTRO_END;
    const alphaScale = themeFromEl(root) === "light" ? 1.4 : 1;
    ...
    if (intro) {
      for (const ray of rays) {
        ...
        ctx.lineTo(
          ray.ox + (ray.tx - ray.ox) * u,
          ray.oy + (ray.ty - ray.oy) * u,
        );
    ...
    for (const [ia, ib] of bonds) {
      ...
      if (intro) {
        ...
        if (vis > 0.08) {
          const fade =
            vis < 0.92 ? vis : Math.max(0, 1 - (elapsed - 2.35) * 1.4);
    ...
    for (const p of particles) {
      ...
      ctx.arc(
        ...
        y,
        p.size *
          (0.55 + 0.45 * z + 0.45 * (intro ? 1 - u * 0.4 : 0)),
        0,
        Math.PI * 2,
      );
  ...
  let boot = loadLogo(img, opts.logoSrc)
    .then(() => {
      if (!alive) return;
      accents = cacheAccents(root);
      measure();
      sample();
      kick();
    })
...
export function useDnaSign(
  rootRef: RefObject<HTMLElement | null>,
  signRef: RefObject<HTMLElement | null>,
  markRef: RefObject<HTMLImageElement | null>,
  fieldRef: RefObject<HTMLCanvasElement | null>,
  ...
  useEffect(() => {
    ...
    if (!root || !sign || !mark || !field || !pulse) return;
    ...
    return startDna({ field, mark, pulse, sign, root, logoSrc });
  }, [rootRef, signRef, markRef, fieldRef, pulseRef, logoSrc]);
}
